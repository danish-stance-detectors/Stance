...
settings_rand = [
    ('rbf-svm', SVC(), {'kernel': ['rbf'], 'gamma': sp_expon(scale=.1), 'C': sp_expon(scale=100),
                        'class_weight': ['balanced', None]}),
    # ('linear-svm', SVC(), {'kernel': ['linear'], 'C': sp_expon(scale=100), 'class_weight': ['balanced', None]}),
    # ('tree', DecisionTreeClassifier(), {'criterion': ['entropy', 'gini'], 'splitter':['best', 'random'],
    ...
    #                                                'class_weight': ['balanced', None],
    #                                                'C': sp_expon(scale=100), 'multi_class': ['auto']}),
    # ('random-forest', RandomForestClassifier(), {'n_estimators': [10, 100, 1000], 'criterion': ['entropy', 'gini'],
...
for name, estimator, tuned_parameters in (settings_rand if rand_search else settings):
    start = time.time()
